...
const FORMAS = {
  M: { el: 'el', senor: 'el señor', aSenor: 'al señor', identificado: 'identificado', trabajador: 'trabajador',
       vinculado: 'vinculado', suscrito: 'El suscrito', interesado: 'el interesado',
       asalariado: 'el asalariado', su: 'del', afiliado: 'afiliado' },
  F: { el: 'la', senor: 'la señora', aSenor: 'a la señora', identificado: 'identificada', trabajador: 'trabajadora',
       vinculado: 'vinculada', suscrito: 'La suscrita', interesado: 'la interesada',
       asalariado: 'la asalariada', su: 'de la', afiliado: 'afiliada' },
  '': { el: 'el(la)', senor: 'el(la) señor(a)', aSenor: 'al(a la) señor(a)', identificado: 'identificado(a)', trabajador: 'trabajador(a)',
...
// Meses equivalentes prestados dentro de un año, a partir de la fecha de inicio.
function mesesEnAnio(fechaInicio, anio) {
  if (!fechaInicio) return 12;
  const [a, m, d] = fechaInicio.split('-').map(Number);
  if (a < anio) return 12;
  if (a > anio) return 0;
  const fraccion = Math.min(1, (31 - d) / 30);
  return Number(((12 - m) + fraccion).toFixed(2));
}

module.exports = {
  MESES, moneda, fechaLarga, periodo, numeroALetras, monedaEnLetras, dato, esc,
  concordancia, mesesEnAnio
};
